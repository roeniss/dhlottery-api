...
def test_create_auto_tickets_makes_tickets_with_specified_count():
    for count in [1, 2, 3, 4, 5]:
        tickets = Lotto645Ticket.create_auto_tickets(count)
        assert len(tickets) == count

def test_create_tickets_makes_tickets_with_specified_size():
    for size in [[''], ['', ''], ['', '', ''], ['', '', '', ''], ['', '', '', '', '']]:
...
def test_create_tickets_failed_with_nondigit_value():
    ...
    try:
        ...
        assert e.args[0] == '숫자를 입력하세요 (입력된 값: a,b,c).'
...
def test_create_tickets_failed_with_duplicated_numbers():
    ...
    try:
        ...
        assert e.args[0] == '중복되지 않도록 숫자들을 입력하세요 (입력된 값: 1,1).'
...
def test_create_tickets_failed_with_under_range_number():
    ...
    try:
        ...
        assert e.args[0] == '각 번호는 1부터 45까지의 숫자만 사용할 수 있습니다 (입력된 값: 0).'
...
def test_create_tickets_failed_with_over_range_number():
    ...
    try:
        ...
        assert e.args[0] == '각 번호는 1부터 45까지의 숫자만 사용할 수 있습니다 (입력된 값: 46).'
...
def test_create_tickets_failed_with_too_many_numbers():
    ...
    try:
        ...
        assert e.args[0] == '숫자는 0개 이상 6개 이하의 숫자를 입력해야 합니다 (입력된 값: 1,2,3,4,5,6,7).'
