...
1. main 브랜치에서 배포할 버전 태그를 생성한 뒤 master 브랜치로 PR을 만듭니다.
2. CI가 통과되고 머지되면 [tag-and-publish 워크플로우](https://github.com/roeniss/dhlottery-api/actions/workflows/tag-and-publish.yml) 를 직접 수행합니다. 이렇게 하면 태그를 기준으로 Pypi 에 새 버전 배포가 진행됩니다.
3. 생성된 태그로 GitHub Release 를 생성합니다.
